...
class Reservoir():
    def __init__(self, cube_shape=(10,10,10), inputs=None, coordinates=None, mapping=None, c=0.4, l=0.169, c_in=0.9, l_in=1.2, use_mps=False):
        ...
        if coordinates is None:
            ...
            x, y, z = torch.meshgrid(torch.linspace(0, 1, cube_shape[0]),
                                     torch.linspace(0, 1, cube_shape[1]),
                                     torch.linspace(0, 1, cube_shape[2]), indexing='xy')
        ...
        if mapping is None:
            input_conn = torch.where(torch.rand(self.n_neurons, inputs) > 0.95,
                                     torch.ones_like(torch.rand(self.n_neurons, inputs)),
                                     torch.zeros(self.n_neurons, inputs)) / 50
        else:
    ...
    def _ensure_3d_input(self, X):
        """
        Ensures the input data is 3D, reshaping if necessary.

        Args:
            X: Input data (2D or 3D).

        Returns:
            torch.Tensor: Input data reshaped to 3D if needed.
        """
        if X.ndim == 2:  # If 2D, reshape to (batch_size, 1, n_features)
            X = X[:, None, :]
        return X
    ...
    def simulate(self, X, mem_thr=0.1, refractory_period=5, train=True, learning_rule=STDP(), verbose=True):
        ...
        """
        X = self._ensure_3d_input(X)  # Ensure input is 3D
