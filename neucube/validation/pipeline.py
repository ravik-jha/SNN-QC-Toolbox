...
class Pipeline():
    ...
    def _ensure_3d_input(self, X):
        ...
        Returns:
            X_reshaped: Input data reshaped to 3D if needed.
        """
        if X.ndim == 2:  # If 2D, reshape to (n_samples, 1, n_features)
            X = X[:, None, :]
        return X
    ...
    def fit(self, X_train, y_train, train=False, learning_rule=STDP(), verbose=False):
        ...
        Returns:
            None
        """
        X_train = self._ensure_3d_input(X_train)  # Ensure input is 3D
    ...
    def predict(self, X_test):
        ...
        """
        X_test = self._ensure_3d_input(X_test)  # Ensure input is 3D
